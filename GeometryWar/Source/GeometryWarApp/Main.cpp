...
VIMPORT IVisPlugin_cl* GetEnginePlugin_GeometryWarEnginePlugin();
VIMPORT IVisPlugin_cl* GetEnginePlugin_vScaleformPlugin();

class GeometryWarApp : public VAppImpl
{
public:
  GeometryWarApp() {}
  virtual ~GeometryWarApp() {}
...
VAPP_IMPLEMENT_SAMPLE(GeometryWarApp);

void GeometryWarApp::SetupAppConfig(VisAppConfig_cl& config)
{
  ...
  config.m_videoConfig.m_iYRes = 1100;  // Set the Window size Y if not in fullscreen.
  ...
  */
  ...
  config.m_videoConfig.m_szWindowTitle = "GeometryWar";

  config.m_videoConfig.m_bWaitVRetrace = true;
}

void GeometryWarApp::PreloadPlugins()
{
  ...
  VISION_PLUGIN_ENSURE_LOADED(GeometryWarEnginePlugin);
...
//---------------------------------------------------------------------------------------------------------
void GeometryWarApp::AfterSceneLoaded(bool bLoadingSuccessful)
{
...
//---------------------------------------------------------------------------------------------------------
bool GeometryWarApp::Run()
{
  return true;
}

void GeometryWarApp::DeInit()
{
  // De-Initialization
  // [...]
}
